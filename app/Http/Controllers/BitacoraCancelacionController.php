...
class BitacoraCancelacionController extends Controller
{
    ...
    {
        $bitacora = BitacoraCancelacion::join('users', 'bitacora_cancelacions.userCreator', '=', 'users.id')
            ->select('users.name', 'bitacora_cancelacions.cSistema', 'bitacora_cancelacions.motivo','bitacora_cancelacions.carts_id')->get();
